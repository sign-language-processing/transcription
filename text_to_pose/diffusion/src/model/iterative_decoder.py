...
from ..metrics.dtw import dynamic_time_warping_mean_joint_error
from ..metrics.mse import masked_mse
...
class IterativeGuidedPoseGenerationModel(pl.LightningModule):
    ...
    def forward(self,
        ...
        assert first_pose.dim() == 2, "First pose should be a single pose, not a sequence"
        ...
        sequence_length = max(1, min(round(float(sequence_length)), self.max_seq_size))
    ...
    def forward_to_body(self, forward_iterator: Iterator, fps=1):
        data = list(forward_iterator)[-1]
        data = torch.unsqueeze(data, 1).cpu()
        conf = torch.ones_like(data[:, :, :, 0])
        return NumPyPoseBody(fps, data.numpy(), conf.numpy())
    ...
    def validation_step(self, batch, *unused_args):
        for i, (pose, pose_data, text) in enumerate(zip(batch["pose"]["obj"], batch["pose"]["data"], batch["text"])):
            first_pose = pose_data[0]
            # TODO batch inference
            iterator = self.forward(text=text, first_pose=first_pose)
            pred_pose_body = self.forward_to_body(iterator)
            # Metrics
            dtw_mje = dynamic_time_warping_mean_joint_error(pose.body, pred_pose_body)
            self.log("validation_dtw_mje", dtw_mje)

            # Visualize and log video
            if i == 0:
                pred_pose = Pose(header=pose.header, body=pred_pose_body)
                pred_pose.body.data *= 50  # Scale up pose
                pred_pose.focus()
                frames = np.stack(list(PoseVisualizer(pred_pose, thickness=1).draw()))
                # Frames must be (time, channel, height, width)
                frames = np.transpose(frames, (0, 3, 1, 2))
                self.logger.experiment.log({"video": [Video(frames, fps=pose.body.fps)]})
    ...
    def step(self, batch, *unused_args, steps: List[int]):
        ...
        self.log(f"{phase}_seq_length_loss", sequence_length_loss, batch_size=batch_size)
        self.log(f"{phase}_refinement_loss", refinement_loss, batch_size=batch_size)
        self.log(f"{phase}_smoothness_loss", smoothness_loss, batch_size=batch_size)
        self.log(f"{phase}_reconstruction_loss", pose_reconstruction_loss, batch_size=batch_size)
