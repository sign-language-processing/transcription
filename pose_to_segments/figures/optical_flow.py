...
# Get the first datum
dataset = get_dataset(poses="holistic", fps=50,
                      components=["POSE_LANDMARKS", "LEFT_HAND_LANDMARKS", "RIGHT_HAND_LANDMARKS"], split="train[:10]")
datum = dataset[8]
print("id", datum["id"])
pose = datum["pose"]["obj"]
bio = datum["bio"]["sentence"].numpy()
...
def hand_normalization(pose: Pose):
